#!/usr/bin/env python3
"""
BalloncordUpdater — checks GitHub for a new Ballooncord release.

Modes:
  (no args)          — Startup check: compare versions, write flag file, exit.
                       Called by Balloncord on startup if "Check for updates" is ON.
  --download v url   — Download mode: show progress window, download, replace exe, launch.
                       Called by Balloncord when the user clicks the update balloon.
"""
...
# ── Startup check (no args) ───────────────────────────────────────────────────

def _check_only_mode() -> None:
    """Check GitHub; write flag if update available, then exit."""
    _hide_console()
...
# ── Download mode (--download version url) ────────────────────────────────────

class UpdaterWindow:
    """XP-style progress window for downloading and applying a Balloncord update."""
